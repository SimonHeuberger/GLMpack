...
#' layout(matrix(c(1,1,2,2,3,3,4,4,5,6,6,7,8,8,8,8), ncol=4, byrow = TRUE), heights = c(0.3,0.3,0.3,0.1))
...
#' segments(c(low.pll, high.pll), c(-55,-55), c(low.pll, high.pll), rep(logLik(dp.out)-qchisq(.95,1)/2,2), lty=3, col="gray40")
...
#'          ylim=c(min(coef.vector[,i])-abs(min(coef.vector[,i]))*0.25, max(coef.vector[,i])+abs(max(coef.vector[,i]))*0.25))
